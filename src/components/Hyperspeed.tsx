...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    const roadMarkings_fragment = `
      ...
      float brokenLines = step(1.0 - brokenLineWidth * uLanes, fract(uv.x * uLanes)) * step(laneEmptySpace, fract(uv.y * 100.));
      brokenLines *= step(0.00 + brokenLineWidth * uLanes, fract(uv.x * uLanes));

      float shoulderLinesWidth = uShoulderLinesWidthPercentage;
      float shoulderLines = step(1.0 - shoulderLinesWidth, uv.x) + step(uv.x, shoulderLinesWidth);

      if(brokenLines > 0.0 || shoulderLines > 0.0){
        color = brokenLines > 0.0 ? uBrokenLinesColor : uShoulderLinesColor;
      }
    `;
